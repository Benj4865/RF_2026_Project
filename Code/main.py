    ...
    if game_state == "running":
        ...
        if game_mode == "single":
            ...
            for obstacle, obstacle_hitbox in zip(obstacles, obstacle_hitboxes):
                ...
                if obstacle_hitbox.bottom < (PLAYER_Y - COOP_COLLISION_ZONE_HEIGHT):
                    continue

                collided_left = obstacle["lane"] == 0 and obstacle_hitbox.colliderect(left_player_ground_hitbox)
                collided_right = obstacle["lane"] == 1 and obstacle_hitbox.colliderect(right_player_ground_hitbox)
